...
public class ApplicationAdminTest extends OSGiTest {
	...
	private ApplicationDescriptor getApplication(String appName) {
		try {
			...
			if (refs == null || refs.length == 0) {
				...
				Assertions.fail("Could not find app pid: " + appName + " available apps are: " + availableApps); //$NON-NLS-1$ //$NON-NLS-2$
			}
			ApplicationDescriptor result = (ApplicationDescriptor) getContext().getService(refs[0]);
			if (result != null)
				getContext().ungetService(refs[0]);
			else
				Assertions.fail("Could not get application descriptor service: " + appName); //$NON-NLS-1$
			return result;
		} catch (InvalidSyntaxException e) {
			Assertions.fail("Could not create app filter", e); //$NON-NLS-1$
		}
		return null;
	}
	...
	public void testSimpleApp() {
		...
		try {
			...
			Assertions.fail("failed to launch simpleApp", e); //$NON-NLS-1$
		}
	...
	private void doInvalidScheduleArgs(ApplicationDescriptor app, String id, Map invalidArgs, String topic, String eventFilter, boolean recurring, boolean invalidKeys, boolean invalidValues) {
		...
		try {
			...
			if (invalidKeys || invalidValues)
				Assertions.fail("Should have failed with invalid arguments"); //$NON-NLS-1$
		} catch (InvalidSyntaxException e) {
			Assertions.fail("Failed to schedule an application", e); //$NON-NLS-1$
	...
	public void testExitValue01() {
		...
		try {
			...
			Assertions.fail("unexpected exception waiting for exit value", e); //$NON-NLS-1$
		}
	...
	public void testExitValue02() {
		...
		try {
			...
			Assertions.fail("unexpected syntax exception for tracker", e); //$NON-NLS-1$
		}
		tracker.open();
		ApplicationHandle handle = null;
		try {
	...
	public void testExitValue06() {
		...
		try {
			...
			Assertions.fail("Expected an ApplicationException for exit value not available"); //$NON-NLS-1$
		} catch (ApplicationException e) {
			assertEquals("Unexpected error type", ApplicationException.APPLICATION_EXITVALUE_NOT_AVAILABLE, e.getErrorCode()); //$NON-NLS-1$
		} catch (InterruptedException e) {
			Assertions.fail("Unexpected interrupted exception waiting for exit value", e); //$NON-NLS-1$
		}
	}
	...
	public void testExitValue07() {
		...
		try {
			...
			Assertions.fail("failed to launcher simpleApp", e); //$NON-NLS-1$
		}
	...
	public void testGlobalSingleton() {
		...
		try {
			ApplicationHandle handle = app.launch(args);
			handle.destroy();
			Assertions.fail("should not be able to launch a global singleton app: " + app.getApplicationId()); //$NON-NLS-1$
		} catch (ApplicationException e) {
		...
		try {
			for (; i <= cardinality; i++) {
				...
				if (i == cardinality && hasMax) {
					Assertions.fail("Expected an ApplicationException.");
				}
			}
		} catch (ApplicationException e) {
			if (!hasMax || i != cardinality)
				Assertions.fail("Unexpected ApplicationException", e); //$NON-NLS-1$
			assertEquals("check error code", ApplicationException.APPLICATION_NOT_LAUNCHABLE, e.getErrorCode()); //$NON-NLS-1$
		} finally {
	...
	private void doTestMainThreaded(String appID) {
		...
		try {
			...
			if (instances.size() == 0)
				Assertions.fail("Unable to launch a main threaded application", e); //$NON-NLS-1$
			assertEquals("check error code", ApplicationException.APPLICATION_NOT_LAUNCHABLE, e.getErrorCode()); //$NON-NLS-1$
		} finally {
			for (Iterator handles = instances.iterator(); handles.hasNext();) {
				((ApplicationHandle) handles.next()).destroy();
	...
	public void testHandleEvents01() throws InvalidSyntaxException {
		...
		try {
			...
			Assertions.fail("failed to launch application", e); //$NON-NLS-1$
		} finally {
			tracker.close();
		}
	}
	...
	public void testDescriptorEvents01() throws InvalidSyntaxException, BundleException {
		...
		try {
			...
			Assertions.fail("Failed to create bundle installer", e); //$NON-NLS-1$
		}
		...
		try {
			...
			try {
				...
				Assertions.fail("failed to launch app: " + app.getApplicationId(), e); //$NON-NLS-1$
			}
	...
	public void testPersistentLock02() {
		...
		try {
			ApplicationHandle handle = app.launch(null);
			handle.destroy();
			Assertions.fail("The application should be locked: " + app.getApplicationId()); //$NON-NLS-1$
		} catch (ApplicationException e) {
	...
	public void testPersistentSchedule02() throws InvalidSyntaxException {
		...
		try {
			...
			try {
				...
				Assertions.fail("Unexpected application exception waiting for an exit value", e); //$NON-NLS-1$
			}
			...
			Assertions.fail("got interupted", e); //$NON-NLS-1$
		} finally {
			tracker.close();
			scheduledApp.remove();
		}
	...
	public void testPersistentSchedule03() {
		...
		if (scheduledApp != null) {
			scheduledApp.remove();
			Assertions.fail("Scheduled application should not be found: " + scheduledApp.getScheduleId()); //$NON-NLS-1$
		}
	}
	...
	public void testPersistentSchedule07() {
		...
		try {
			app.schedule("schedule.duplicate1", null, "org/osgi/application/timer", "(minute=*)", true); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			Assertions.fail("should not be able to create a duplicate scheduled application id"); //$NON-NLS-1$
		} catch (InvalidSyntaxException e) {
			Assertions.fail("Failed to schedule an application", e); //$NON-NLS-1$
		} catch (ApplicationException e) {
	...
	private ScheduledApplication getScheduleApplication(String scheduleID, boolean failOnMissing) {
		try {
			...
			if (refs == null || refs.length == 0) {
				...
				Assertions.fail("Could not find schedule: " + scheduleID + " available apps are: " + availableScheds); //$NON-NLS-1$ //$NON-NLS-2$
			}
			ScheduledApplication result = (ScheduledApplication) getContext().getService(refs[0]);
			if (result != null)
				getContext().ungetService(refs[0]);
			else
				Assertions.fail("Could not get scheduled application: " + scheduleID); //$NON-NLS-1$
			return result;
		} catch (InvalidSyntaxException e) {
			Assertions.fail("Could not create app filter", e); //$NON-NLS-1$
		}
	...
	public static class ApplicationHandleTracker implements ServiceTrackerCustomizer {
		...
		public synchronized void waitForEvent(String instance, String state) {
			...
			if (!found) {
				...
				Assertions.fail("failed waiting for handle event: " + instance + " " + state + foundEventsMsg); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
	...
	public static class ApplicationDescriptorTracker implements ServiceTrackerCustomizer {
		...
		public synchronized void waitForEvent(String pid, String type, Boolean locked, Boolean launchable) {
			...
			if (!found) {
				...
				Assertions.fail("failed waiting for descriptor event: " + pid + " " + type + " " + locked + " " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
						+ launchable + foundEventsMsg);
			}
		}
