	...
	static void doLock(String testLocationDir, boolean release, boolean succeed) {
		...
		try {
			...
			if (succeed ? testLocation.isLocked() : !testLocation.isLocked())
				Assertions.fail("location should " + (succeed ? "not " : "") + "be locked");
			if (succeed ? !testLocation.lock() : testLocation.lock())
				Assertions.fail((succeed ? "Could not" : "Could") + " lock location");
			if (!testLocation.isLocked())
				Assertions.fail("location should be locked");
		} catch (MalformedURLException e) {
			Assertions.fail("failed to create the location URL", e);
		} catch (IOException e) {
			Assertions.fail("failed to lock with IOExcetpion", e);
		} finally {
	...
	static void doRelease() {
		try {
			if (lockedTestLocation == null)
				Assertions.fail("lockedTestLocation == null !!");
			if (!lockedTestLocation.isLocked())
				Assertions.fail("lockedTestLocation is not locked!!");
			lockedTestLocation.release();
			if (lockedTestLocation.isLocked())
				Assertions.fail("lockedTestLocation is still locked!!");
		} catch (IOException e) {
			Assertions.fail("failed to unlock lockedTestLocation", e);
		} finally {
