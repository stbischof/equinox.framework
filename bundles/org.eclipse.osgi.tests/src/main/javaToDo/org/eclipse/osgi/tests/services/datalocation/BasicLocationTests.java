...
public class BasicLocationTests extends CoreTest {
	...
	public void testCreateLocation01() {
		...
		try {
			...
			Assertions.fail("Failed to set location", t);
		}
		try {
			assertTrue("Could not lock location", testLocation.lock());
		} catch (IOException e) {
			Assertions.fail("Failed to lock location", e);
		}
	...
	public void testCreateLocation02() {
		...
		try {
			assertTrue("Could not lock location", testLocation.lock());
			testLocation.release();
			Assertions.fail("Should not be able to lock read-only location");
		} catch (IOException e) {
			// expected
		}
	}
	...
	private void doAllTestLocationDataArea(Location location, String dataAreaPrefix) {
		...
		try {
			...
			Assertions.fail("Failed to set location", e);
		}
		...
		try {
			...
			Assertions.fail("expected failure when location is not set");
		} catch (IOException e) {
			// expected;
		}
	}
	...
	private void doTestLocateDataArea(Location location, String dataAreaPrefix, String namespace) {
		...
		try {
			...
			Assertions.fail("Failed to get data area.", e);
		}
	...
	public void testSetLocationWithEmptyLockFile() {
		...
		try {
			...
			Assertions.fail("Failed to set location", t); //$NON-NLS-1$
		}
		try {
			assertTrue("Could not lock location", testLocation.isLocked()); //$NON-NLS-1$
		} catch (IOException e) {
			Assertions.fail("Failed to lock location", e); //$NON-NLS-1$
		}
		testLocation.release();
	}
